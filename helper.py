...
def fetch_medal_tally(df, year, country):
    ...
    if year == 'Overall' and country == 'Overall':
        temp_df = medal_df  # Use '=' for assignment
        ...
        temp_df = medal_df[(medal_df['Year'] == year) & (medal_df['region'] == country)]  # Fixed the hardcoded values

    if flag == 1:
        x = temp_df.groupby('Year', as_index=False)[['Gold', 'Silver', 'Bronze']].sum().sort_values('Year').reset_index(
            drop=True)
    else:
        x = temp_df.groupby('region', as_index=False)[['Gold', 'Silver', 'Bronze']].sum().sort_values('Gold',
...
def medal_tally(df):
    ...
    medal_tally = medal_tally.groupby('region', as_index=False)[['Gold', 'Silver', 'Bronze']].sum().sort_values('Gold',
                                                                                                                ascending=False).reset_index(
        drop=True)
...
def data_over_time(df, col):  # this give df named as participating_nations_over_time
...
def most_successful(df, sport):
    ...
    # Drop duplicates to ensure unique Name-Sport-Region mapping
    athlete_info = df[['Name', 'Sport', 'region']].drop_duplicates(subset=['Name'])

    # Merge with athlete information
